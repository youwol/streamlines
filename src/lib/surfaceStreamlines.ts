...
export class StreamLinesOnSurface {
            ...
            istep?: number})
    {
        ...
        if (this.first_calculus_) {
            ...
            this.integStep_ = Math.max(bbox.xLength, bbox.yLength, bbox.zLength) / istep
        ...
        */
        if (this.seedAttr !== undefined) {
            ...
            for (let i=0; i<faces.length; ++i) {
                const f = faces[i]
                const val = attr[f.id]
                if (val > this.seedThresholdUp || val < this.seedThresholdDown) {
                    f['skip'] = true
                }
            }
        }
    ...
    private tooNear(iP: vec.Vector3): boolean {
        const min = [iP[0] - this.d_sep_, iP[1] - this.d_sep_, iP[2] - this.d_sep_] as vec.Vector3
        const max = [iP[0] + this.d_sep_, iP[1] + this.d_sep_, iP[2] + this.d_sep_] as vec.Vector3
        let tooNear = false
        ...
        maxIter: number): void
    {
        ...
        if (!comp_bbox.contains(bbox) || this.borderEdges_.includes(entryEdge)) {
            return
        }
        ...
        edgesOfPoly.push(polygon.halfedge, polygon.halfedge.next, polygon.halfedge.next.next)
        for (let i=0; i<edgesOfPoly.length; ++i) {
            ...
            if (edge != entryEdge) {
                ...
                if (curSeg.intersectRay(segment, iC, distances) == true &&
                    (distances.dist > 0) && 
