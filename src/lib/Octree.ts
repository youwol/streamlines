...
export class Octree {
    ...
    get rootBBox() {return this.root_.bbox} // <-------------

    getCellCenter(cell: Cell = this.root_): vec.Vector3 {
        return vec.scale(vec.add(vec.clone(cell.min), cell.max), 0.5) as vec.Vector3
        // return cell.min.clone().add(cell.max).scale(0.5)
    }

    getItemsNear(pt: vec.Vector3, items: Array<vec.Vector3>, check_mother: boolean): void {
        const root_bbox = this.bbox_
        items = []
    ...
    private subdivide_cell(cell: Cell, max_depth: number, max_nb_items_in_cell: number) {
...
class Cell {
    ...
    constructor(depth: number, mother: Cell, min: vec.Vector3, max: vec.Vector3) {
        this.mother_ = mother
        this.depth_ = depth
        this.min_   = min
        this.max_   = max
    ...
    get mother() {return this.mother_}
    get depth() {return this.depth_}
    get min() {return this.min_}
    get max() {return this.max_}
    get nb_items(): number {return this.items_.length}
    get has_children(): boolean {return this.children_[0] !== undefined}
    get bbox() {return new BBox(this.min_, this.max_)} // <------ optim ?
    ...
    do_subdivide(max_depth: number, max_nb_items_in_cell: number) {
        if (this.depth >= max_depth  || (this.has_children == false && this.nb_items <= max_nb_items_in_cell)) {
        ...
        if (this.has_children === false) {
            ...
            this.children_[0] = new Cell(this.depth_+1, this, min, createV3(half_x, half_y, half_z) as vec.Vector3)
            this.children_[1] = new Cell(this.depth_+1, this, createV3(half_x, min[1], min[2]), createV3(max[0], half_y, half_z))
            this.children_[2] = new Cell(this.depth_+1, this, createV3(half_x, half_y, min[2]), createV3(max[0], max[1], half_z))
            this.children_[3] = new Cell(this.depth_+1, this, createV3(min[0], half_y, min[2]), createV3(half_x, max[1], half_z))
            this.children_[4] = new Cell(this.depth_+1, this, createV3(min[0], min[1], half_z), createV3(half_x, half_y, max[2]))
            this.children_[5] = new Cell(this.depth_+1, this, createV3(half_x, min[1], half_z), createV3(max[0], half_y, max[2]))
            this.children_[6] = new Cell(this.depth_+1, this, createV3(half_x, half_y, half_z), createV3(max[0], max[1], max[2]))
            this.children_[7] = new Cell(this.depth_+1, this, createV3(min[0], half_y, half_z), createV3(half_x, max[1], max[2]))
