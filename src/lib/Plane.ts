...
const orient3dfast = (pa: Array<number>, pb: Array<number>, pc: Array<number>, pd: Array<number>) => {
    let adx, bdx, cdx
    let ady, bdy, cdy
    let adz, bdz, cdz
...
export class Plane {
    ...
    set(p1: vec.Vector3, p2: vec.Vector3, p3: vec.Vector3): Plane {
        ...
        this.m_d = -(this.m_normal[0]*this.m_p1[0])-(this.m_normal[1]*this.m_p1[1])-(this.m_normal[2]*this.m_p1[2])
        return this
    }
    ...
    static fromPointAndNormal(p: vec.Vector3, normal: vec.Vector3): Plane {
        ...
        let vec2 = createV3()
        if (Math.abs(y) > 1.E-03) {
            vec2 = vec.normalize( setV3(vec2, -y, x-((z*z)/y), z) ) as vec.Vector3
            // vec2.set(-y, x-((z*z)/y), z).normalize()
        } else if (Math.abs(x) > 1.E-03) {
            vec2 = vec.normalize( setV3(vec2, y-((z*z)/x), -x, z) ) as vec.Vector3
            // vec2.set(y-((z*z)/x), -x, z).normalize()
        } else if (Math.abs(z) > 1.E-03) {
            vec2 = vec.normalize( setV3(vec2, x, -z, y-((x*x)/z)) ) as vec.Vector3
        ...
        return new Plane(p, vec.add(p, vec2) as vec.Vector3, vec.add(p, vec3) as vec.Vector3)
    }
    ...
    toUV(p: vec.Vector3, result: vec.Vector2, normalize_coords?: boolean): boolean {
    ...
    inPlane(pt: vec.Vector3): boolean {
        ...
        const mean_length_sq = (vec.norm2(p12) + vec.norm2(p13) + vec.norm2(p23)) / 3
        const dot = vec.dot(n, vec.create(this.m_p1, pt))
        const dot_sq = dot*dot
        if (dot >= 0) {
	        const signed_distance_sq = dot_sq / vec.norm2(n)
	        return ((signed_distance_sq/mean_length_sq) < 1.E-12)
        } else {
	        const signed_distance_sq = -dot_sq/vec.norm2(n)
	        return ((-signed_distance_sq/mean_length_sq) < 1.E-12)
        }
    ...
    private toUVFromVector(v: vec.Vector3, result: vec.Vector2): boolean {
        ...
        const p1_plus_projected_vector = vec.add(vec.clone(this.m_p1), projected_vector) as vec.Vector3
        const p1_plus_projected_vector_uv = createV2()
        const r2 = this.toUV(p1_plus_projected_vector, p1_plus_projected_vector_uv, false)
